...
${whitespace}</${node3.tag}>`;
  ...
  function getAttribute(query, attribute3) {
    const element = document.querySelector(query);
    if (element === null) {
      return Err({ value: `Failed to find ${query}` });
    } else {
      return Ok({ value: element.getAttribute(attribute3) || element[attribute3] });
    }
  }
  function Click(args) {
    return __spreadValues({
  ...
  function testPlanHelper(xs, plan2) {
    const _res3835 = xs;
            ...
            case "Click": {
              ...
              switch (_res677072758.kind) {
                case "Ok": {
                  ...
                  return testPlanHelper(ys, Success({}));
                }
                case "Err": {
                  const { value } = _res677072758;
                  return Failure({
  ...
  function testPlan(actions, afterPlan) {
    const plan2 = testPlanHelper(actions, Success({}));
    const runAfter = afterPlan();
    return plan2;
  }
  ...
  function update(msg, model, send) {
    ...
    switch (_res108417.kind) {
      ...
      case "RunTests": {
        const blank = console.log(testPlan(plan, function(_) {
          return send(Noop({}));
        }));
        return model;
      }
  ...
  var viewTestRunner = button2([onClick(function(_) {
    return RunTests({});
  })], [], [text2("Run tests")]);
  function view(model) {
    return div2([], [], [button2([onClick(function(_) {
      return Clicked({});
    })], [], [text2(`${model.number}`)]), viewTestRunner]);
  }
